...
def main():
    ...
    with opener(filename) as vcf:
        ...
        for line in vcf:
            ...
            # Find out the ploidy of the genotypes, just distinguishes if all the sample are diploid or not
            # Note that only the first locus is checked to save time and memory
            elif not line.startswith("#") and line.strip("\n") != "":
                broken = line.strip("\n").split("\t")
                if len(sample_names) == len(broken) - 9:
                    ploidy_list = []
                    for j in range(9, len(broken)):
                        ploidy_list += [(len(broken[j].split(":")[0]) // 2) + 1]
